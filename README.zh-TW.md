...
* **PIN 碼保護編輯權限**：瀏覽行程無需登入，新增、修改、刪除、調整同時間事件順序與上傳票券等編輯功能則受 PIN 碼保護。其他裝置會在 30 秒內自動同步變更，或在畫面回到前景時立即更新。
...
| 環境變數 | 預設值 | 說明 |
|---|---|---|
| `TRIPCOMPANION_PIN` | **無預設值 —— 未設定或仍為範本值 `CHANGEME` 時程式拒絕啟動** | 共用的編輯 PIN 碼 |
...
> **瀏覽行程不需要登入，這也包含票券附件。** 附件的儲存檔名雖採用不可猜測的隨機 UUID 字串，但網頁上會直接附上連結，因此任何取得網址的人都能開啟。在對外公開前，若要上傳護照掃描檔或簽證等隱私文件，請務必在 hostname 前端配置存取控制政策（例如 Cloudflare Access）。

共用的 PIN 碼採用簡短的數字形式，其定位是用於防止誤觸修改，而非直接暴露於公開網路的第一道防線；請將存取控制政策（Access Policy）視為真正的安全邊界。PIN 碼防暴力的速率限制（Rate limiting）使用 10 分鐘滑動視窗：單一 IP 上限 10 次失敗，並設有較高（400 次）的全局總量防護門檻，刻意避免單一攻擊者耗盡共用計數器而導致所有編輯者被鎖定在外。
